...
@register
class MolDETRCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        self._validate_target_labels(target_classes_o)
    ...
    def _validate_target_labels(self, labels):
        if labels.numel() == 0:
            return
        invalid = (labels < 0) | (labels >= self.num_classes)
        if invalid.any():
            bad = labels[invalid]
            raise ValueError(
                f'Found target label(s) outside [0, {self.num_classes - 1}]. '
                f'Observed min={labels.min().item()}, max={labels.max().item()}, '
                f'invalid_sample={bad[:10].tolist()}. '
                'Please make sure `num_classes` matches the dataset category ids.'
            )
