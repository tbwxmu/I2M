...
def get_ocr(force_cpu):
    ...
    from .paddleocr_compat import create_ocr
    params = {
        'use_angle_cls': True,
        'lang': 'en',
        'det_db_thresh': 0.5,
    ...
    ocr = create_ocr(**params)
    return ocr 
class CaptionRemover:
    def __init__(self, config = None, force_cpu = False, remove_captions=True, use_ocr=True):
        self.config = config
        self.border_size = 30
    ...
    def _preprocess_images_process(self, images_filenames):
        ...
        for image_filename in tqdm(images_filenames):
            ...
            if self.remove_captions and self.use_ocr and self.ocr is not None:
                try:
                    image = self(pil_image)
                    pil_image = Image.fromarray(image).convert('RGB')
                except Exception as e:
                    print(f"Warning: Failed to remove captions: {e}")
